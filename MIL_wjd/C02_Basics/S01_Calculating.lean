...
example : (a + b) * (c + d) = a * c + a * d + b * c + b * d := by
  ...
  rw [add_assoc (a * c), add_comm (b * c) (a * d), ←add_assoc]
...
example : (a + b) * (c + d) = a * c + a * d + b * c + b * d := by
  calc
    ...
    _ = a * c + a * d + b * c + b * d := by
      rw [add_comm (b * c) (a * d), ← add_assoc]

example (a b : ℝ) : (a + b) * (a - b) = a ^ 2 - b ^ 2 := by
  rw [mul_sub, add_mul, add_mul, mul_comm b a]
  rw [←add_sub, ←sub_sub, sub_self, pow_two]
  rw [pow_two, add_sub, add_zero]
...
example (a b : ℝ) : (a + b) * (a - b) = a ^ 2 - b ^ 2 := by
  calc
    ...
    _ = a * a + (a * b - a * b) - b * b := by rw [add_sub]
