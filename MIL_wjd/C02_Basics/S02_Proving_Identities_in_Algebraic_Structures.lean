...
theorem add_right_cancel {a b c : R} (h : a + b = c + b) : a = c := by
  rw [←add_neg_cancel_right a b, h, add_neg_cancel_right c b]
...
theorem neg_neg (a : R) : - -a = a := by
  rw [←eq_neg_of_add_eq_zero (add_right_neg a)]
...
theorem self_subℝ (a : ℝ) : a - a = 0 :=
  by apply add_right_neg

theorem self_subℝ' (a : ℝ) : a - a = 0 :=
  by exact add_right_neg a

theorem one_add_one_eq_two : 1 + 1 = (2 : R) := by
  norm_num

theorem two_mul (a : R) : 2 * a = a + a := by
  have h : 2 * a = (1 + 1) * a := by rw [one_add_one_eq_two]
  rw [h, add_mul, one_mul]

theorem two_mul_calc (a : R) : 2 * a = a + a := by
  calc
    2 * a = (1 + 1) * a := by rw [one_add_one_eq_two]
    _ = a + a := by rw [add_mul, one_mul]
...
theorem idemp (a : G) (h : a = a * a) : a = 1 := by
  calc
    a = 1 * a := by rw [one_mul]
...
theorem mul_cancel_right {a b c : G} (h : a * c = b * c) : a = b := by
  calc
    a = a * 1 := by rw [mul_one]
...
theorem inv_def (a b : G)(h : b * a = 1) : a⁻¹ = b := by
  have h' : a⁻¹ * a = b * a  := by rw [h, ←mul_left_inv]
  apply (mul_cancel_right h')

theorem mul_inv_rev (a b : G) : (a * b)⁻¹ = b⁻¹ * a⁻¹ := by
  have h : (b⁻¹ * a⁻¹) * (a * b) = 1 := by
    rw [mul_assoc, ←mul_assoc a⁻¹, mul_left_inv, one_mul, mul_left_inv]
  apply (inv_def (a * b) (b⁻¹ * a⁻¹)  h)
